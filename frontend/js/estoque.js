...
const estoqueElementos = {
  ...
  validadeCampo: document
    .querySelector("#movementExpiration")
    ?.closest(".modal-field"),
...
function configurarUsuario() {
  ...
  const perfil = String(usuario?.perfil || "OPERADOR_ESTOQUE");
...
function preencherCategorias(categorias) {
  ...
  categorias.forEach((categoria) => {
    estoqueElementos.categoria.appendChild(
      new Option(categoria.nome, categoria.id),
    );
  });
  estoqueElementos.categoria.value = categorias.some(
    (categoria) => String(categoria.id) === valorAtual,
  )
    ? valorAtual
    : "";
}
...
function preencherLocalizacoes(localizacoes) {
  estoqueElementos.localizacao.replaceChildren(
    new Option("Selecione uma localização", ""),
  );
  localizacoes.forEach((localizacao) => {
    estoqueElementos.localizacao.appendChild(
      new Option(localizacao.nome, localizacao.id),
    );
  });
}
...
function renderizarResumo() {
  ...
  estoqueElementos.total.textContent = window.BabycareUI.formatarNumero(
    resumo.estoqueTotal,
  );
  estoqueElementos.baixo.textContent = window.BabycareUI.formatarNumero(
    resumo.estoqueBaixo,
  );
  estoqueElementos.semEstoque.textContent = window.BabycareUI.formatarNumero(
    resumo.semEstoque,
  );
  estoqueElementos.movimentacoesHoje.textContent =
    window.BabycareUI.formatarNumero(resumo.movimentacoesHoje);

  const quantidadeAtencao =
    Number(resumo.estoqueBaixo || 0) + Number(resumo.semEstoque || 0);
...
function obterProdutosFiltrados() {
  const termo = String(estoqueElementos.busca?.value || "")
    .trim()
    .toLowerCase();
  ...
  const filtrados = estoqueDados.produtos.filter((produto) => {
    const pesquisavel =
      `${produto.nome} ${produto.codigo} ${produto.categoria}`.toLowerCase();
    return (
      (!termo || pesquisavel.includes(termo)) &&
  ...
  return filtrados.sort((a, b) => {
    if (ordenacao === "estoque-menor")
      return a.estoque - b.estoque || a.nome.localeCompare(b.nome, "pt-BR");
    if (ordenacao === "estoque-maior")
      return b.estoque - a.estoque || a.nome.localeCompare(b.nome, "pt-BR");
    if (ordenacao === "movimentacao")
      return (
        new Date(b.ultimaMovimentacao || 0) -
        new Date(a.ultimaMovimentacao || 0)
      );
...
function criarLinhaProduto(produto) {
  ...
  const nomeProduto = String(produto.nome || "Produto");
  avatar.textContent = nomeProduto.charAt(0).toUpperCase();
  ...
  const statusProduto = String(produto.status || "NORMAL");
  status.className = `estoque-status ${statusProduto.toLowerCase().replaceAll("_", "-")}`;
  status.textContent =
    statusProduto === "SEM_ESTOQUE"
      ? "Sem estoque"
      : statusProduto === "BAIXO"
        ? "Estoque baixo"
        : "Normal";
  ...
  linha.append(
    ...
    criarCelula(
      window.BabycareUI.formatarNumero(produto.estoque),
      "estoque-quantidade",
    ),
    criarCelula(
      window.BabycareUI.formatarNumero(produto.estoqueMinimo),
      "estoque-minimo",
    ),
    statusCelula,
...
function renderizarTabela() {
  ...
  estoqueElementos.corpoTabela.replaceChildren(
    ...produtos.map(criarLinhaProduto),
  );
...
async function carregarEstoque() {
  ...
  try {
    ...
    estoqueDados = {
      ...
      produtos: (Array.isArray(dados.produtos) ? dados.produtos : []).map(
        (produto) => {
          const estoque = Number(produto.estoque ?? produto.estoqueAtual ?? 0);
      ...
      movimentacoes: Array.isArray(dados.movimentacoes)
        ? dados.movimentacoes
        : [],
    ...
    estoqueElementos.alertaTexto.textContent =
      erro.message || "Não foi possível carregar os dados reais do estoque.";
    window.BabycareUI.mostrarToast(
      erro.message || "Não foi possível carregar o estoque.",
      "error",
    );
  } finally {
...
function preencherProdutosModal() {
  const produtosAtivos = estoqueDados.produtos.filter(
    (produto) => produto.statusProduto === "ATIVO",
  );
  estoqueElementos.produto.replaceChildren(
    new Option("Selecione um produto", ""),
  );
  produtosAtivos.forEach((produto) => {
    estoqueElementos.produto.appendChild(
      new Option(`${produto.nome} — ${produto.codigo}`, produto.id),
    );
  });
}
...
function preencherMotivos(tipo) {
  ...
  estoqueElementos.motivo.replaceChildren(
    ...motivos.map(([valor, texto]) => new Option(texto, valor)),
  );
}

function atualizarInformacaoProduto() {
  const produto = estoqueDados.produtos.find(
    (item) => String(item.id) === estoqueElementos.produto.value,
  );
...
function atualizarRegraLote(tipo) {
  ...
  estoqueElementos.loteLabel.querySelector("span").textContent = entrada
    ? "*"
    : "opcional";
  estoqueElementos.loteLabel.querySelector("span").className = entrada
    ? ""
    : "optional-label";
...
function abrirModal(tipo, produtoId = "") {
  ...
  estoqueElementos.modalTitulo.textContent =
    tipo === "ENTRADA" ? "Registrar entrada" : "Registrar saída";
  estoqueElementos.modalDescricao.textContent =
    tipo === "ENTRADA"
  ...
  estoqueElementos.enviar.textContent =
    tipo === "ENTRADA" ? "Registrar entrada" : "Registrar saída";
...
function validarMovimentacao() {
  ...
  if (tipo === "ENTRADA" && !estoqueElementos.lote.value.trim())
    return "Informe o número do lote para registrar uma entrada.";
  if (tipo === "ENTRADA" && !estoqueElementos.localizacao.value)
    return "Selecione a localização do lote recebido.";
  if (
    tipo === "ENTRADA" &&
    produto.possuiValidade &&
    !estoqueElementos.validade.value
  )
    return "Informe a validade do lote para este produto.";
  ...
  if (
    estoqueElementos.motivo.value === "AJUSTE" &&
    !estoqueElementos.observacao.value.trim()
  ) {
...
async function registrarMovimentacao(evento) {
  ...
  const tipo = estoqueElementos.tipo.value;
  const motivo =
    estoqueElementos.motivo.options[estoqueElementos.motivo.selectedIndex]
      ?.textContent || "";
  const payload = {
    tipo,
    ...
    localizacaoId: estoqueElementos.localizacao.value
      ? Number(estoqueElementos.localizacao.value)
      : null,
  };
  ...
  try {
    ...
    window.BabycareUI.mostrarToast(
      "Movimentação registrada no estoque real.",
      "success",
    );
  } catch (erroMovimentacao) {
    mostrarErro(
      erroMovimentacao.message || "Não foi possível registrar a movimentação.",
    );
  } finally {
...
function configurarEventos() {
  [
    estoqueElementos.busca,
    estoqueElementos.categoria,
    estoqueElementos.status,
    estoqueElementos.ordenacao,
  ].forEach((elemento) => elemento.addEventListener("input", renderizarTabela));

  estoqueElementos.abrirSidebar?.addEventListener("click", () =>
    document.body.classList.add("sidebar-open"),
  );
  estoqueElementos.fecharSidebar?.addEventListener("click", () =>
  ...
  estoqueElementos.notificacao?.addEventListener("click", () => {
    const alertas =
      Number(estoqueDados.resumo?.estoqueBaixo || 0) +
      Number(estoqueDados.resumo?.semEstoque || 0);
    window.BabycareUI.mostrarToast(
      alertas
        ? `${alertas} produto(s) precisam de atenção no estoque.`
  ...
  estoqueElementos.topbarBusca?.addEventListener("search", () => {
    if (estoqueElementos.topbarBusca.value.trim()) {
      window.BabycareUI.mostrarToast(
        "Use a busca de estoque para filtrar os produtos.",
      );
    }
  });

  estoqueElementos.sidebarBusca?.addEventListener("input", () => {
    const termo = String(estoqueElementos.sidebarBusca?.value || "")
      .trim()
      .toLocaleLowerCase("pt-BR");
    document.querySelectorAll(".main-nav .nav-link").forEach((item) => {
      item.hidden =
        Boolean(termo) &&
        !item.textContent.toLocaleLowerCase("pt-BR").includes(termo);
    });
  });

  estoqueElementos.entrada.addEventListener("click", () =>
    abrirModal("ENTRADA"),
  );
  estoqueElementos.saida.addEventListener("click", () => abrirModal("SAIDA"));
  estoqueElementos.produto.addEventListener(
    "change",
    atualizarInformacaoProduto,
  );
  ...
  estoqueElementos.verHistorico.addEventListener("click", () =>
    document
      .querySelector(".movimentacoes-panel")
  ...
  document.addEventListener("keydown", (evento) => {
    if (evento.key === "Escape" && !estoqueElementos.modal.hidden)
      fecharModal();
  });
  ...
  document.querySelectorAll("[data-coming-soon]").forEach((link) => {
    link.addEventListener("click", (evento) => {
      evento.preventDefault();
      window.BabycareUI.mostrarToast(
        `O módulo ${link.dataset.comingSoon} será desenvolvido na próxima etapa.`,
      );
    });
  });
}
