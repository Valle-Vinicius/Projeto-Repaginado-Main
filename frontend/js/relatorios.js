...
function num(v) {
  return window.BabycareUI.formatarNumero(Number(v || 0), 3)
    .replace(/,?0+$/, "")
    .replace(/,$/, "");
}
...
function preencherFiltros() {
  const catAtual = el.category.value,
    prodAtual = el.product.value;
  el.category.innerHTML = '<option value="">Todas as categorias</option>';
  el.product.innerHTML = '<option value="">Todos os produtos</option>';
  dados.categorias.forEach((x) =>
    el.category.appendChild(new Option(x.nome, x.id)),
  );
  dados.produtos.forEach((x) =>
    el.product.appendChild(new Option(`${x.nome} — ${x.codigo}`, x.id)),
  );
...
function renderChart() {
  ...
  el.chartEmpty.hidden = serie.some(
    (x) => Number(x.entradas || 0) > 0 || Number(x.saidas || 0),
  );
}
...
function render() {
  ...
  el.movements.textContent = window.BabycareUI.formatarNumero(
    r.movimentacoes || 0,
  );
  el.entries.textContent = num(r.entradas);
  el.exits.textContent = num(r.saidas);
  el.products.textContent = window.BabycareUI.formatarNumero(
    r.produtosMovimentados || 0,
  );
  renderChart();
  renderCategories();
  renderProducts();
...
function atualizarUsuario() {
  ...
  const perfil = String(u.perfil || "Perfil de acesso")
    .replace(/_/g, " ")
    .toLocaleLowerCase("pt-BR")
    .replace(/(^|\s)\S/g, (x) => x.toLocaleUpperCase("pt-BR"));
  const parts = nome.split(/\s+/).filter(Boolean);
  const ini = (
    parts.length > 1 ? parts[0][0] + parts.at(-1)[0] : nome.slice(0, 2)
  ).toUpperCase();
...
function configurar() {
  atualizarUsuario();
  [el.period, el.type, el.category, el.product].forEach((x) =>
    x.addEventListener("change", carregar),
  );
    ...
    document
      ...
      .forEach(
        (x) =>
          (x.hidden =
            Boolean(t) &&
            !x.textContent.toLocaleLowerCase("pt-BR").includes(t)),
      );
  });
  el.bell?.addEventListener("click", () =>
    window.BabycareUI.mostrarToast(
      "Relatório atualizado a partir dos dados do banco.",
    ),
  );
